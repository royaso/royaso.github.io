...
还发现一点,即使我文章引用的是图床里的图片,但在github上现实的图片地址却是`githubusercontent.com'的.不管了.这才导致加载有点慢.



github上的图片压缩服务是生效了,但时间有点长,压缩率也不是很高就17%,最关键的是它还要合并分支才行.不过这也解了我的疑惑.毕竟压缩后的图片的sha id肯定不一样了.
...
`https://github.com/royaso/imgs/blob/master/2019-03-29-%E8%8B%B9%E6%9E%9C%E7%AC%94%E8%AE%B0%E6%9C%AC_alfred_%E6%A8%8A%E8%83%9C%E7%BE%8E_typora.md/%E6%A8%8A%E8%83%9C%E7%BE%8E.jpg?raw=true`

会自动跳转到下面各个地址

`<https://raw.githubusercontent.com/royaso/imgs/master/2019-03-29-%E8%8B%B9%E6%9E%9C%E7%AC%94%E8%AE%B0%E6%9C%AC_alfred_%E6%A8%8A%E8%83%9C%E7%BE%8E_typora.md/%E6%A8%8A%E8%83%9C%E7%BE%8E.jpg>`
...
1. 压缩电脑上的手机图片,来清理空间`xnview`批处理
   1. `/Applications/XnConvert.app`
   2. xargs
   3. 想想,要是命令行好用的话就容易多了

2. 原来xnconvert是xnview的一个独立功能,还带有命令行,所以批量处理的时候还是需要的
...
1. 大内存就是爽啊,cpu都快占满了,电脑还是运行流畅,风扇也是中度旋转.
2. 图片真是占空间啊,几乎500个图谱处理完后就能省出一个G啊,我12万图片应该能省出25G,从原来的7G,等结束的时候应该有35G吧.就能安装xcode了.这也是执念啊,我并没有一定要安装xcode的原因,就是看到了mac app store里的最新xcode不能安装在我这个落后了两个小版本的osx上,才费劲去官网下载的.这里不得不吐槽下了
   1. 搜索到官网可以下载,你要注册开发者账号登陆后才能下载,
   2. 本来6G的文件当然想用迅雷之类的加速下载,可是下载地址不是真实地址.所以,我是用chrome浏览器自带的下载功能,我出去逛一圈回来差不多下载好了.本来以为不会关闭屏幕的: 我把屏幕关闭和系统休眠都设置为无限,忘了还有屏保20分钟后开启,已开启就Lock屏幕了
   3. 安装这个xpi的xcode要先解压,硬盘不够就解压不了,至少要20G空间,所以才压缩图片来清理出空间
3. 怎么批量处理图片
   1. 直接用xnview,发觉真是好用啊,可以选择图片后`command-u`,也可以添加目录,浏览界面还有"显示目录下的所有文件"功能
   2. 要有效压缩图片文件,我只是简单处理下,其实你不设置"动作",软件也会压缩的,设置的地方还有点隐蔽,在最后输出的界面`format`格式点开才能看到各种图片类型的压缩比率,我设置jpg为75%,应该就够了默认是80%,但我为了更省点空间.结果也没看,应该不会被压缩得很惨
   3. 直接删除原文件.本来保守点应该是输出到另一个目录.但我空间不够了,再说了,难道我还一个个去检查啊,只能胆大心细了.对了,还考虑到,如果处理时中断了,就不会分不清哪些是处理过的哪些没处理.
   4. 要做到删除原文件,要设置文件名不去更改,然后勾选`删除原文件`
   5. 至于最关键的`动作`设置为最长的边为1024像素,而且只缩小不拉大.
4. 我去现在还有11万图片没处理,我的硬盘已经空出20G了.啧啧啧.

5. 麻蛋!数据又损失了,转换的gif都不动了,难怪压缩率那么高,操.看来还是宁可事前多做准备啊,事后损失太惨了
6. 还有那些长图也都压缩的看不清了.失策了
7. 不采取措施这种事情还会发生,如果在不吸取教训.
8. 存一份就好.就跟两个手表,反而搞不清楚以哪个为准
